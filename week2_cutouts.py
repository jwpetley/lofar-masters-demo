...
from astropy.visualization import PercentileInterval, SqrtStretch, ZScaleInterval
import os
...
def plot_cutouts(cutouts):
    fig, axs = plt.subplots(1, len(cutouts))

    transform = ZScaleInterval() + SqrtStretch()
    for i, cutout in enumerate(cutouts):
        trans_data = transform(cutout.data)

        axs[i].imshow(trans_data, origin = 'lower')
        axs[i].axis('off')
...
if __name__== "__main__":
    ...
    for source in tqdm(edfn_table_facet):
        ...
        #     plot_cutouts([cutout_05, cutout_07, cutout_15, cutout_60])
